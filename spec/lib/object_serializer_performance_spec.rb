...
describe FastJsonapi::ObjectSerializer, performance: true do
  ...
  include_context 'ams_dev movie class'
  ...
  def print_stats(message, count, ams_time, ams_dev_time, jsonapi_time, our_time)
    format = '%-15s %-10s %s'
    puts ''
    puts message
    puts format(format, 'Serializer', 'Records', 'Time')
    puts format(format, 'AMS serializer', count, ams_time.round(2).to_s + ' ms')
    puts format(format, 'AMS-dev serializer', count, ams_dev_time.round(2).to_s + ' ms')
  ...
  def run_hash_benchmark(message, movie_count, our_serializer, ams_serializer, ams_dev_serializer, jsonapi_serializer)
    ...
    ams_dev_time = Benchmark.measure { ams_hash = ams_dev_serializer.as_json }.real * 1000
    jsonapi_time = Benchmark.measure { ams_hash = jsonapi_serializer.to_hash }.real * 1000

    print_stats(message, movie_count, ams_time, ams_dev_time, jsonapi_time, our_time)
  end

  def run_json_benchmark(message, movie_count, our_serializer, ams_serializer, ams_dev_serializer, jsonapi_serializer)
    our_json = nil
    ams_json = nil
    jsonapi_json = nil
  ...
  context 'when comparing with other serializer gems' do
    [1, 25, 250, 1000].each do |movie_count|
      ...
      it "should serialize #{movie_count} records atleast #{speed_factor} times faster than AMS" do
        ...
        ams_dev_serializer = AMSDevCollectionSerializer.new(build_ams_dev_movies(movie_count), AMSDevMovieSerializer, {}, :no_links)
        jsonapi_serializer = JSONAPISerializer.new(jsonapi_movies)

        message = "Serialize to JSON string #{movie_count} records"
        our_json, ams_json = run_json_benchmark(message, movie_count, our_serializer, ams_serializer, ams_dev_serializer, jsonapi_serializer)

        message = "Serialize to Ruby Hash #{movie_count} records"
        run_hash_benchmark(message, movie_count, our_serializer, ams_serializer, ams_dev_serializer, jsonapi_serializer)
  ...
  context 'when comparing with AMS 0.10.x and with includes and meta' do
    [1, 25, 250, 1000].each do |movie_count|
      ...
      it "should serialize #{movie_count} records atleast #{speed_factor} times faster than AMS" do
        ...
        ams_dev_serializer = AMSDevCollectionSerializer.new(build_ams_dev_movies(movie_count), AMSDevMovieSerializer, options, :no_links)
